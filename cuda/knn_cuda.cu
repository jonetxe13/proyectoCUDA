...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    ...
    printf("\t(hautazkoa) prozesatu nahi den hitz kopurua -- (opcional) num de "
           "palabras que se quieren procesar\n");
    exit(-1);
    ;
  }
  ...
  // Idatzi antzekotasunak similarities fitxategietan -- Escribe las similitudes
  // en el fichero similarities
  f2 = fopen(argv[2], "w");
  if (f2 == NULL) {
    printf("Errorea %s fitxategia irekitzean -- Error abriendo fichero\n",
           argv[2]);
  ...
  printf("antzekotasunak idatzita fitxategietan -- similitudes escritas en el "
         "fichero\n");
