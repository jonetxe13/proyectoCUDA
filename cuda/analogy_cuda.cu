...
int main(int argc, char *argv[]) {
  ...
  printf("Sartu analogoak diren bi hitzak eta analogia bilatu nahi diozun "
         "hitza: \n");
  printf("Introduce las dos palabras analogas y la palabra a la que le quieres "
         "buscar la analogia: \n");
  ...
  find_closest_word(result_vector, words, numwords, idx1, idx2, idx3,
                    &closest_word_idx, &max_similarity, numBloques, numHilos);

  // clock_gettime (CLOCK_REALTIME, &t1);
  if (closest_word_idx != -1) {
    printf("\nClosest_word: %s (%d), sim = %f \n", dictionary[closest_word_idx],
