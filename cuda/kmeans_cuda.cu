...
// EXEKUTATZEKO: kmeans embeddings.dat dictionary.dat myclusters.dat [numwords]
...
struct clusterinfo // clusterrei buruzko informazioa -- informacion de los
// clusters
{
...
void initialize_centroids(float *words, float *centroids, int n,
...
// void update_centroids(float *words, float *centroids, int *wordcent,
...
__global__ void k_means_calculate(float *words, int numWords, int dim,
                                  int numclusters, int *wordcent,
  ...
  int stride = blockDim.x * gridDim.x;
  localChanged = 0;
  for (i = idx; i < numWords;
       i +=
       stride) { // por cada palabra, a lo mejor mas de una palabra por hilo...
    maxDist = -DBL_MAX;
    centroideFinal = wordcent[i];       // asignamos la palabra anterior
    for (j = 0; j < numclusters; j++) { // y por cada centroide
      // dist=word_distance (&words[i*dim],&centroids[j*dim]);//miro la
      // distancia...
    ...
    if (wordcent[i] !=
        centroideFinal) // despues me quedo el mas cerca y lo guardo en wordcent
    {
      localChanged = 1;
...
double centroid_homogeneity(float *centroids, int i, int numclusters) {
  ...
  for (j = 0; j < numclusters; j++) {
    if (i != j)
      distancia +=
          word_distance(&centroids[i * EMB_SIZE], &centroids[j * EMB_SIZE]);
  }
  return distancia;
}
...
double validation(float *words, struct clusterinfo *members, float *centroids,
  ...
  for (i = 0; i < numclusters; i++) {
    ...
    cent_homog[i] =
        disbat / (numclusters - 1); // si son 5 clusters, se han sumado 4 dist.
  }
...
int main(int argc, char *argv[]) {
  int i, j, numwords, k, iter, changed, end_classif, numBloques, numHilos;
  ...
  int *cu_wordcent;
  float *cu_centroids;
  int *cu_cluster_sizes;

  int *cu_changed;
  cudaMalloc(&cu_changed, sizeof(int));

  if (argc < 4) {
    printf("\nCall: kmeans embeddings.dat dictionary.dat myclusters.dat "
           "[numwords][nhilos],[nbloques]\n\n");
    printf("\t(in) embeddings.dat and dictionary.dat\n");
    printf("\t(out) myclusters.dat\n");
    printf("\t(nhilos optional) ");
    printf("\t(nbloques optional)");
    printf("\t(numwords optional) prozesatu nahi den hitz kopurua -- num de ");
    exit(-1);
    ;
  }

  // Irakurri datuak sarrea-fitxategietatik -- Leer los datos de los ficheros de
  // entrada
  // ===================================================================================
  ...
  cudaMalloc(&cu_wordcent, numwords * sizeof(int));
  ...
  while (numclusters < NUMCLUSTERSMAX && end_classif == 0) {
    ...
    for (iter = 0; iter < MAX_ITER; iter++) {
      ...
      cudaMemcpy(cu_wordcent, wordcent, numwords * sizeof(int),
                 cudaMemcpyHostToDevice);
      // centroids(embedings de los centroides)
      cudaMemcpy(cu_centroids, centroids,
                 numclusters * EMB_SIZE * sizeof(float),
                 cudaMemcpyHostToDevice);
      k_means_calculate<<<numBloques, numHilos>>>(cu_words, numwords, EMB_SIZE,
                                                  numclusters, cu_wordcent,
                                                  cu_centroids, cu_changed);
      cudaDeviceSynchronize();
      cudaMemcpy(&changed, cu_changed, sizeof(int), cudaMemcpyDeviceToHost);
      // todo
      cudaMemcpy(wordcent, cu_wordcent, numwords * sizeof(int),
                 cudaMemcpyDeviceToHost); // esto hara falta
      cudaDeviceSynchronize();
      if (changed == 0)
    ...
    // cluster bakoitzeko hitzak (osagaiak) eta kopurua -- palabras de cada
    // clusters y cuantas son
    for (i = 0; i < numwords; i++) {
      cluster = wordcent[i];
      zenb = members[cluster].number;
      members[cluster].elements[zenb] =
          i; // clusterreko hitza -- palabra del cluster
