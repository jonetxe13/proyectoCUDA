...
__global__ void k_means_calculate(float *words, int numWords, int dim,
		int numclusters, int *wordcent,float *centroids, int *changed)
	{
	...
	localChanged=0;
	for (i = idx; i < numWords; i += stride) {// por cada palabra, a lo mejor mas de una palabra por hilo...
		maxDist = -DBL_MAX;
...
double cluster_homogeneity(float *words, struct clusterinfo *members, int i,
	 ...
	 ****************************************************************************************/
	int *resultados =(int*) calloc(members[i].number * members[i].number,
