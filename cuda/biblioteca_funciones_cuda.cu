...
void find_closest_word(float *result_vector, float *words, int numWords,
                       int idx1, int idx2, int idx3, int *closest_word_idx,
                       float *max_similarity, int numBloques, int numHilos) {
  int i;
  *max_similarity = 0.0;
...
__global__ void reduc_analogy(int idx1, int idx2, int idx3, int numWords,
                              int *indicesBloques, float *maximosBloques,
                              float *words, float *result_vector) {
  int tid = threadIdx.x;
  ...
  __shared__ float maximos[MAX_THREADS_PER_BLOCK];
  ...
  // ahora tenemos los mas grandes entre los que le ha tocado procesar a este
  // thread
  __syncthreads();
  for (stride = 1; stride < blockDim.x / 2; stride *= 2) {
    if ((tid % (2 * stride)) == 0) {
      if (maximos[tid + stride] > maximos[tid]) {
        maximos[tid] = maximos[tid + stride];
        indices[tid] = indices[tid + stride];
      }
      // esto no hace obviamente falta obviamente, pero lo dejo porque se
      // entiende un poco mejor la logica: else{ maximos[tid]=maximos[tid];
  ...
  if (tid == 0) {
    indicesBloques[blockIdx.x] = indices[0];
    maximosBloques[blockIdx.x] = maximos[0];
  }
}
...
void knn_complet(float *words, int numwords, float *similarities) {
  ...
  knn_cuda<<<numBloques, tamBloques>>>(wordsCuda, numwordsCuda,
                                       similaritiesCuda);
