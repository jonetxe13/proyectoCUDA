...
__global__ void reduc_analogy(int idx1,int idx2,int idx3,int numWords,int *
		indicesBloques, float* maximosBloques,float * words,float * result_vector)
{
	int tid = threadIdx.x;
	...
	for (i = idx;i<numWords;i+=stride){
		simil =cosine_similarity(result_vector,&words[i*EMB_SIZE],EMB_SIZE);		if(simil>maxSimil&& i !=idx1&& i !=idx2&& i !=idx3){ 		
			maxSimil= simil;
			indice = i;
		}
	}
